...
# Trailing single cable-box suffix (A/B/C) following a digit, e.g. "CLD16A".
_SINGLE_BOX_SUFFIX = re.compile(r'(?<=\d)([A-C])$')


def coupler_base_name(name: str) -> Optional[str]:
    """
    Derive the coupler/base switch name for a per-cable-box device name.

    A bus coupler (ElmCoup) that represents both cable boxes is modelled in
    PowerFactory with the base name only (e.g. "CLD16"), whereas IPS names the
    relay records per cable box ("CLD16A", "CLD16B", "CLD16A+B"). This returns
    the base name so those records can also be indexed under the coupler name.

    Args:
        name: The IPS switch/device name (already split on "_").

    Returns:
        The base name with the cable-box suffix removed, or None if the name
        has no such suffix (i.e. it is already a base/coupler name).

    Examples:
        >>> coupler_base_name("CLD16A+B")
        'CLD16'
        >>> coupler_base_name("CLD16A")
        'CLD16'
        >>> coupler_base_name("CLD16")
        >>> coupler_base_name("NIP1")
    """
    # Combined names (A+B, A+B+C, ...) reuse the canonical suffix list.
    for suffix, _ in SUFFIX_EXPANSIONS:
        if name.endswith(suffix):
            return name[:-len(suffix)]

    # Single cable-box names: strip a trailing A/B/C that follows a digit.
    match = _SINGLE_BOX_SUFFIX.search(name)
    if match:
